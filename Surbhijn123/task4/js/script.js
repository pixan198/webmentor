...
for(i=0; i< calBtn.length;i++) {
   calBtn[i].addEventListener('click', perform);
}

function perform() {

	//The value enter by user will store in "userValues"
	...
	if(!isNaN(userValue)) {
		...
		resetNo = false;
		decimalCount = 0;
		var lastChar = fetchLastCharacter(result.value);
		...
		try {
				var expRes = math.evaluate(result.value);
